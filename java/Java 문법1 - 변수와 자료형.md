...
- `참조형`
  ...
  **클래스 자료형**으로, JDK에서 제공되는 클래스와 프로그래머가 정의하는 클래스가 있을 수 있다. 
...
- **모든 리터럴은 상수 풀에 저장**되어 있다. 
  ...
  - **상수풀은 자바 프로세스의 종료까지 계속 유지되는 메모리 영역**으로 기본적으로 JVM에서 관리하며 프로그래머가 작성한 상수에 대해서 우선적으로 찾아보고 없으면 상수풀에 추가한 이후 그 주소값을 리턴한다. 
  - 즉, 메모리 절약 효과를 위해 사용!
